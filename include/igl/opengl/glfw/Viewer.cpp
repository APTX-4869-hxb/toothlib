...
IGL_INLINE int Viewer::launch(bool resizable /*= true*/,
                              bool fullscreen /*= false*/,
                              const std::string &name, int windowWidth /*= 0*/,
                              int windowHeight /*= 0*/) {
...
IGL_INLINE int Viewer::launch_init(bool resizable, bool fullscreen,
                                   const std::string &name, int windowWidth,
                                   int windowHeight) {
    ...
    if (fullscreen) {
        ...
        window = glfwCreateWindow(mode->width, mode->height, name.c_str(),
                                  monitor, nullptr);
        ...
        if (windowHeight <= 0 && core_list.size() == 1 &&
            core().viewport[3] > 0)
            windowHeight = core().viewport[3];
        else if (windowHeight <= 0)
            windowHeight = 800;
        window = glfwCreateWindow(windowWidth, windowHeight, name.c_str(),
                                  nullptr, nullptr);
    }
    if (!window) {
        glfwTerminate();
...
IGL_INLINE Viewer::Viewer()
    : data_list(1), selected_data_index(0), next_data_id(1),
      selected_core_index(0), next_core_id(2) {
    window = nullptr;
...
Viewer::load_mesh_from_file(const std::string &mesh_file_name_string, bool align_cam) {
    ...
    if (extension == "obj" || extension == "OBJ") {
        ...
        if (!(igl::readOBJ(mesh_file_name_string, V, UV_V, corner_normals, F,
                           UV_F, fNormIndices))) {
            return false;
        }
...
IGL_INLINE bool
Viewer::save_mesh_to_file(const std::string &mesh_file_name_string) {
    // first try to load it with a plugin
    for (unsigned int i = 0; i < plugins.size(); ++i)
    ...
    if (extension == "off" || extension == "OFF") {
        ...
        return igl::writeOBJ(mesh_file_name_string, data().V, data().F,
...
IGL_INLINE bool Viewer::key_pressed(unsigned int unicode_key, int modifiers) {
    ...
    case 's': {
        if (core().is_directional_light) {
            ...
            if (core().is_shadow_mapping) {
                ...
                // The light_position when !is_directional_light is interpretted
                // as a position relative to the _eye_ (not look-at) position of
                // the camera.
                //
                // Meanwhile shadows only current work in is_directional_light
                // mode.
                //
                // If the user wants to flip back and forth between [positional
                // lights without shadows] and [directional lights with shadows]
                // then they can high-jack this key_pressed with a callback.
                //
                // Until shadows support positional lights, let's switch to
                // directional lights here and match the direction best as
                // possible to the current light position.
                core().is_directional_light = true;
                core().light_position =
                    core().light_position + core().camera_eye;
    ...
    case '<':
    case '>': {
        selected_data_index = (selected_data_index + data_list.size() +
                               (unicode_key == '>' ? 1 : -1)) %
                              data_list.size();
        return true;
    }
    case '{':
    case '}': {
        selected_core_index = (selected_core_index + core_list.size() +
                               (unicode_key == '}' ? 1 : -1)) %
                              core_list.size();
        return true;
    }
...
IGL_INLINE bool Viewer::mouse_down(MouseButton button, int modifier) {
    ...
    if (data().V.rows() > 0) {
        ...
        center.head(data().V.cols()) =
            data().V.colwise().sum() / data().V.rows();
    }
...
IGL_INLINE bool Viewer::mouse_move(int mouse_x, int mouse_y) {
    ...
    if (down) {
        ...
        // into viewport-mouse-click coordinates for igl::trackball and
        ...
        case MouseMode::Rotation: {
            ...
            case ViewerCore::ROTATION_TYPE_TRACKBALL:
                igl::trackball(core().viewport(2), core().viewport(3), 2.0f,
                               down_rotation, down_mouse_x - core().viewport(0),
                               down_mouse_y -
                                   (height_window - core().viewport(1) -
                                    core().viewport(3)),
                               mouse_x - core().viewport(0),
                               mouse_y - (height_window - core().viewport(1) -
                                          core().viewport(3)),
            ...
            case ViewerCore::ROTATION_TYPE_TWO_AXIS_VALUATOR_FIXED_UP:
                igl::two_axis_valuator_fixed_up(
                    ...
                    down_mouse_y - (height_window - core().viewport(1) -
                                    core().viewport(3)),
                    mouse_x - core().viewport(0),
                    mouse_y - (height_window - core().viewport(1) -
                               core().viewport(3)),
        ...
        case MouseMode::Translation: {
            ...
            Eigen::Vector3f pos1 = igl::unproject(
                Eigen::Vector3f(mouse_x, core().viewport[3] - mouse_y,
                                down_mouse_z),
                core().view, core().proj, core().viewport);
        ...
        case MouseMode::Zoom: {
            float delta =
                0.001f * (mouse_x - down_mouse_x + mouse_y - down_mouse_y);
...
IGL_INLINE void Viewer::draw() {
    ...
    auto highdpiw_tmp =
        (width_window == 0 || width == 0) ? highdpiw : (width / width_window);
    auto highdpih_tmp = (height_window == 0 || height == 0)
                            ? highdpih
                            : (height / height_window);

    if (fabs(highdpiw_tmp - highdpiw) > 1e-8 ||
        fabs(highdpih_tmp - highdpih) > 1e-8) {
...
template <typename T>
IGL_INLINE void
Viewer::draw_buffer(igl::opengl::ViewerCore &core,
    ...
    unsigned int depth_buffer;
    {
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                               GL_TEXTURE_2D_MULTISAMPLE, color_buffer, 0);
        // create a (also multisampled) renderbuffer object for depth and
        // stencil attachments
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, framebuffer);
    // Clear the buffer
    glClearColor(core.background_color(0), core.background_color(1),
                 core.background_color(2), core.background_color(3));
    ...
    // Copy back in the given Eigen matrices
    {
        typedef typename std::conditional<std::is_floating_point<T>::value,
                                          GLfloat, GLubyte>::type GLType;
        GLenum type =
            std::is_floating_point<T>::value ? GL_FLOAT : GL_UNSIGNED_BYTE;
...
IGL_INLINE void Viewer::post_resize(int w, int h) {
    if (core_list.size() == 1) {
        ...
        // It is up to the user to define the behavior of the post_resize()
        // function when there are multiple viewports (through the
        // `callback_post_resize` callback)
    }
    for (unsigned int i = 0; i < plugins.size(); ++i) {
...
IGL_INLINE ViewerCore &Viewer::core(unsigned core_id /*= 0*/) {
    ...
    assert((core_index >= 0 && core_index < core_list.size()) &&
...
IGL_INLINE bool Viewer::erase_core(const size_t index) {
    assert((index >= 0 && index < core_list.size()) &&
           "index should be in bounds");
    assert(data_list.size() >= 1);
    if (core_list.size() == 1) {
...
IGL_INLINE int Viewer::append_core(Eigen::Vector4f viewport,
    ...
    core_list.push_back(core()); // copies the previous active core and only
...
template void igl::opengl::glfw::Viewer::draw_buffer<unsigned char>(
    ...
    Eigen::Matrix<unsigned char, -1, -1, 0, -1, -1> &,
    Eigen::Matrix<unsigned char, -1, -1, 0, -1, -1> &,
    Eigen::Matrix<unsigned char, -1, -1, 0, -1, -1> &,
    Eigen::Matrix<unsigned char, -1, -1, 0, -1, -1> &,
    Eigen::Matrix<unsigned char, -1, -1, 0, -1, -1> &);
template void igl::opengl::glfw::Viewer::draw_buffer<double>(
    igl::opengl::ViewerCore &, Eigen::Matrix<double, -1, -1, 0, -1, -1> &,
    Eigen::Matrix<double, -1, -1, 0, -1, -1> &,
    Eigen::Matrix<double, -1, -1, 0, -1, -1> &,
    Eigen::Matrix<double, -1, -1, 0, -1, -1> &,
    Eigen::Matrix<double, -1, -1, 0, -1, -1> &);
template void igl::opengl::glfw::Viewer::draw_buffer<float>(
    igl::opengl::ViewerCore &, Eigen::Matrix<float, -1, -1, 0, -1, -1> &,
